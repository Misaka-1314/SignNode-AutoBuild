...
async def get_new_version():
    ...
    except (aiohttp.client_exceptions.ClientConnectorError, TimeoutError, asyncio.exceptions.TimeoutError, aiohttp.client_exceptions.ServerDisconnectedError) as e:
        raise e
    except aiohttp.client_exceptions.ClientResponseError as e:
...
async def stop_reason(num, uid, name, bind_email, email, port):
    try:
        ...
        encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}签到异常停止，停止原因为{reason}"}))
        ...
        if res["data"]["otherId"] == 2:
            ...
            if res["data"]["ifrefreshewm"] == 1 and res["data"]["ifopenAddress"] == 1:
                ...
                if address is not None:
                    ...
                    BACKGROUND_TASKS.add(task)
                    task.add_done_callback(BACKGROUND_TASKS.discard)
                    encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}该签到为{res['data']['ewmRefreshTime']}秒自动更新且指定了签到地点的二维码签到{activetype_append_text}，请使用微信小程序“WAADRI的扫码工具”扫描学习通签到二维码来完成自动签到，小程序使用教程见https://cx-static.waadri.top/QRCodeSign.php"}))
                    await send_message(encrypt)
                    ...
                    BACKGROUND_TASKS.add(task)
                    task.add_done_callback(BACKGROUND_TASKS.discard)
                    encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}该签到为{res['data']['ewmRefreshTime']}秒自动更新且指定了签到地点的二维码签到{activetype_append_text}，但无法获取指定位置信息，请使用微信小程序“WAADRI的扫码工具”选择指定位置并扫描学习通签到二维码来完成自动签到，小程序使用教程见https://cx-static.waadri.top/QRCodeSign.php"}))
                    await send_message(encrypt)
                ...
                BACKGROUND_TASKS.add(task)
                task.add_done_callback(BACKGROUND_TASKS.discard)
                encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}该签到为{res['data']['ewmRefreshTime']}秒自动更新且未指定签到地点的二维码签到{activetype_append_text}，请使用微信小程序“WAADRI的扫码工具”扫描学习通签到二维码来完成自动签到，小程序使用教程见https://cx-static.waadri.top/QRCodeSign.php"}))
                await send_message(encrypt)
                ...
                if address is not None:
                    ...
                    BACKGROUND_TASKS.add(task)
                    task.add_done_callback(BACKGROUND_TASKS.discard)
                    encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}该签到为无自动更新且指定了签到地点的二维码签到{activetype_append_text}，请使用微信小程序“WAADRI的扫码工具”扫描学习通签到二维码来完成自动签到，小程序使用教程见https://cx-static.waadri.top/QRCodeSign.php"}))
                    await send_message(encrypt)
                    set_latitude = latitude
                    set_longitude = longitude
                    set_address = address
                    ...
                    BACKGROUND_TASKS.add(task)
                    task.add_done_callback(BACKGROUND_TASKS.discard)
                    encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}该签到为无自动更新且指定了签到地点的二维码签到{activetype_append_text}，但无法获取指定位置信息，请使用微信小程序“WAADRI的扫码工具”选择指定位置并扫描学习通签到二维码来完成自动签到，小程序使用教程见https://cx-static.waadri.top/QRCodeSign.php"}))
                    await send_message(encrypt)
                ...
                BACKGROUND_TASKS.add(task)
                task.add_done_callback(BACKGROUND_TASKS.discard)
                encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}该签到为无自动更新且未指定签到地点的二维码签到{activetype_append_text}，请使用微信小程序“WAADRI的扫码工具”扫描学习通签到二维码来完成自动签到，小程序使用教程见https://cx-static.waadri.top/QRCodeSign.php"}))
                await send_message(encrypt)
            ...
            if res["data"]["otherId"] == 0:
                ...
                sign_type = "手势签到"
                encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}该签到为手势签到{activetype_append_text}，开始请求爆破服务器对签到手势码进行爆破，爆破时间可能较长，请耐心等待，每日0点至6点爆破服务器不工作，爆破失败为正常现象"}))
                await send_message(encrypt)
                ...
                if signCode is not None:
                    ...
                    encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}签到手势码爆破失败"}))
                    await send_message(encrypt)
                    ...
                    BACKGROUND_TASKS.add(task)
                    task.add_done_callback(BACKGROUND_TASKS.discard)
                    encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}自动签到失败，失败原因为“无法获取签到手势”"}))
                    await send_message(encrypt)
                ...
                if res["data"]["ifopenAddress"] == 1:
                    ...
                    if address is not None:
                        ...
                        encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}该签到为指定位置签到{activetype_append_text}，但无法获取指定位置信息"}))
                        await send_message(encrypt)
                        ...
                        BACKGROUND_TASKS.add(task)
                        task.add_done_callback(BACKGROUND_TASKS.discard)
                        encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}自动签到失败，失败原因为“无法获取指定位置信息”"}))
                        await send_message(encrypt)
                ...
                LOGGER.info(f"{uid}-{name}:该签到为签到码签到{activetype_append_text}，开始请求爆破服务器对签到手势码进行爆破")
                sign_type = "签到码签到"
                encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}该签到为签到码签到{activetype_append_text}，开始请求爆破服务器对签到码进行爆破，爆破时间可能较长，请耐心等待，每日0点至6点爆破服务器不工作，爆破失败为正常现象"}))
                await send_message(encrypt)
                sign_code_info = await get_sign_code_info(uid, name, str(aid))
                signCode = sign_code_info["code"]
                event_time = datetime.datetime.strftime(datetime.datetime.now(), "[%Y-%m-%d %H:%M:%S]")
                if signCode is not None:
                    ...
                    encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}签到码爆破失败"}))
                    await send_message(encrypt)
                    ...
                    BACKGROUND_TASKS.add(task)
                    task.add_done_callback(BACKGROUND_TASKS.discard)
                    encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}自动签到失败，失败原因为“无法获取签到码”"}))
                    await send_message(encrypt)
                ...
                try:
                    ...
                    if ifNeedVCode:
                        ...
                        if validate is not None:
                            ...
                            task.add_done_callback(BACKGROUND_TASKS.discard)
                            event_time = datetime.datetime.strftime(datetime.datetime.now(), "[%Y-%m-%d %H:%M:%S]")
                            encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}自动签到失败，签到需要进行安全验证且无法通过验证，请使用官方节点进行签到或自行登录学习通APP进行签到"}))
                            await send_message(encrypt)
    ...
    for i in range(2):
        ...
        if i == 0:
            if SIGN_INFO_DICT["location"].get(aid):
                ...
                encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "get_sign_location_info", "uid": uid, "name": name, "aid": str(aid)}))
                await send_message(encrypt)
        else:
            if LOCATION_LOCK.locked():
                LOCATION_LOCK.release()
    ...
    for i in range(2):
        ...
        if i == 0:
            if SIGN_INFO_DICT["code"].get(aid):
                ...
                encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "get_sign_code_info", "uid": uid, "name": name, "aid": str(aid)}))
                await send_message(encrypt)
        else:
            if CODE_LOCK.locked():
                CODE_LOCK.release()
    ...
    for i in range(2):
        ...
        if i == 0:
            if SIGN_INFO_DICT["validate"].get(uid) and SIGN_INFO_DICT["validate"][uid].get(aid):
                ...
                encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "get_sign_validate_info", "uid": uid, "name": name, "aid": str(aid)}))
                await send_message(encrypt)
        else:
            if VALIDATE_LOCK.locked():
                VALIDATE_LOCK.release()
                ...
                if data["type"] == "start_sign":
                    ...
                    for k, d in list(QRCODE_SIGN_DICT.items()):
                        if d["start_timestamp"]+86400 < int(time.time()):
                            ...
                            BACKGROUND_TASKS.add(task)
                            task.add_done_callback(BACKGROUND_TASKS.discard)
                            task = asyncio.create_task(send_wechat_message(uid, f"[二维码签到结果：签到取消]\n[签到监测时间] {d['event_time2']}\n[对应课程或班级] {lesson_name}\n[签到活动名称] {d['name_one']}\n[签到类型] {d['sign_type']}{activetype_append_text}\n[签到开始时间] {d['start_time']}\n[签到结束时间] {d['end_time']}\n[签到持续时间] {d['timelong']}\n{signout_wechat_append_text}[签到状态] 监测到签到发布时长超过24小时，系统将不再对当前二维码签到活动进行签到"))
                            BACKGROUND_TASKS.add(task)
                            task.add_done_callback(BACKGROUND_TASKS.discard)
                            event_time = datetime.datetime.strftime(datetime.datetime.now(), "[%Y-%m-%d %H:%M:%S]")
    ...
    while True:
        try:
            async with websockets.connect("wss://cx-wss.waadri.top/othernode_server/websocket", ping_interval=10, ping_timeout=30, max_size=2**22, ssl=SSL_CONTEXT) as sign_server_ws:
                ...
                data = {"t": t, "device_id": encrypt, "uuid": NODE_CONFIG["uuid"], "qrcode_sign_list": temp_list, "password": NODE_CONFIG["node"]["password"], "version": NODE_VERSION, "limit_number": NODE_CONFIG["node"]["limit"], "support_email": NODE_CONFIG["email"]["address"] != "", "night_monitor": NODE_CONFIG["night_monitor"], "token": "0035e056938cf9577ce849afed892963"}
                encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode(data))
                await send_message(encrypt)
...
async def qrcode_sign_handle(session, keys, name, schoolid, courseid, classid, aid, uid, qrcode_info, address, longitude, latitude, lesson_name, is_numing, sign_num, event_time2, name_one, sign_type, start_time, end_time, timelong, header, devicecode, activetype_append_text, signout_email_append_text, signout_wechat_append_text, port):
    try:
            ...
            try:
                ...
                if txt == "请先登录再进行签到":
                    ...
                    if validate_text["result"]:
                        await record_debug_log(uid+"-"+name+":签到需要进行安全验证，aid:"+str(aid))
                        ...
                        if validate is None:
                            if not QRCODE_SIGN_DICT[keys]["sign_status"]:
                                ...
                                BACKGROUND_TASKS.add(task)
                                task.add_done_callback(BACKGROUND_TASKS.discard)
                                task = asyncio.create_task(send_wechat_message(uid, f"[学习通二维码签到结果：签到失败]\n[签到监测时间] {event_time2}\n[对应课程或班级] {lesson_name}\n[签到活动名称] {name_one}\n[签到类型] {sign_type}{activetype_append_text}\n[签到开始时间] {start_time}\n[签到结束时间] {end_time}\n[签到持续时间] {timelong}\n{signout_wechat_append_text}[签到状态] 签到失败，签到需要进行安全验证且无法通过验证，系统将不再对当前二维码签到活动进行签到，请使用官方节点进行签到或自行登录学习通APP进行签到"))
                                BACKGROUND_TASKS.add(task)
                                task.add_done_callback(BACKGROUND_TASKS.discard)
                                event_time = datetime.datetime.strftime(datetime.datetime.now(), "[%Y-%m-%d %H:%M:%S]")
                                encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}通过微信小程序云端共享获取到课程或班级“{lesson_name}”的二维码签到的二维码与指定位置信息，但自动签到失败，签到需要进行安全验证且无法通过验证，系统将不再对当前二维码签到活动进行签到，请使用官方节点进行签到或自行登录学习通APP进行签到"}))
                                await send_message(encrypt)
        ...
        if txt == "success":
            if not QRCODE_SIGN_DICT[keys]["sign_status"]:
                ...
                BACKGROUND_TASKS.add(task)
                task.add_done_callback(BACKGROUND_TASKS.discard)
                task = asyncio.create_task(send_wechat_message(uid, f"[学习通二维码签到结果：签到成功]\n[签到监测时间] {event_time2}\n[对应课程或班级] {lesson_name}\n[签到活动名称] {name_one}\n[签到类型] {sign_type}{activetype_append_text}\n[签到开始时间] {start_time}\n[签到结束时间] {end_time}\n[签到持续时间] {timelong}\n{signout_wechat_append_text}[签到状态] 收到同班同学提交的签到二维码，签到成功"))
                BACKGROUND_TASKS.add(task)
                task.add_done_callback(BACKGROUND_TASKS.discard)
                USER_LIST[uid]["success_sign_num"] += 1
                event_time = datetime.datetime.strftime(datetime.datetime.now(), "[%Y-%m-%d %H:%M:%S]")
                ...
                if is_numing and USER_LIST[uid]["success_sign_num"] >= sign_num:
                    ...
                    BACKGROUND_TASKS.add(task)
                    task.add_done_callback(BACKGROUND_TASKS.discard)
                    task = asyncio.create_task(send_wechat_message(uid, f"[学习通在线自动签到系统停止监控通知]\n[监控停止时间] {event_time2}\n[监控停止原因] 定次签到模式完成指定成功签到次数\n如需重新启动签到监控请登录学习通在线自动签到系统并重新启动签到监控"))
                    BACKGROUND_TASKS.add(task)
                    task.add_done_callback(BACKGROUND_TASKS.discard)
                    encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}定次签到模式已完成指定成功签到次数"}))
                    await send_message(encrypt)
            ...
            if not QRCODE_SIGN_DICT[keys]["sign_status"]:
                ...
                task.add_done_callback(BACKGROUND_TASKS.discard)
                event_time = datetime.datetime.strftime(datetime.datetime.now(), "[%Y-%m-%d %H:%M:%S]")
                encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}收到同班同学提交的课程或班级“{lesson_name}”的签到二维码与指定位置信息，但学习通提示“您已签到过了”，系统将不再对当前二维码签到活动进行签到"}))
                await send_message(encrypt)
            ...
            if not QRCODE_SIGN_DICT[keys]["sign_status"]:
                ...
                task.add_done_callback(BACKGROUND_TASKS.discard)
                event_time = datetime.datetime.strftime(datetime.datetime.now(), "[%Y-%m-%d %H:%M:%S]")
                encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}收到同班同学提交的课程或班级“{lesson_name}”的签到二维码与指定位置信息，但学习通提示“非本班学生”，系统将不再对当前二维码签到活动进行签到"}))
                await send_message(encrypt)
            ...
            if not QRCODE_SIGN_DICT[keys]["sign_status"]:
                ...
                task.add_done_callback(BACKGROUND_TASKS.discard)
                event_time = datetime.datetime.strftime(datetime.datetime.now(), "[%Y-%m-%d %H:%M:%S]")
                encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}收到同班同学提交的课程或班级“{lesson_name}”的签到二维码与指定位置信息，但学习通提示“同一设备不允许重复签到”，系统将不再对当前二维码签到活动进行签到"}))
                await send_message(encrypt)
            ...
            if txt == "errorLocation1" or txt == "errorLocation2":
                event_time = datetime.datetime.strftime(datetime.datetime.now(), "[%Y-%m-%d %H:%M:%S]")
                encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}收到同班同学提交的课程或班级“{lesson_name}”的签到二维码与指定位置信息，但自动签到失败，失败原因为“{txt}”，您所选位置可能不在教师指定签到位置范围内，请使用微信小程序重新选择指定位置并扫描未过期的签到二维码，扫描后系统将继续尝试签到"}))
                await send_message(encrypt)
            else:
                event_time = datetime.datetime.strftime(datetime.datetime.now(), "[%Y-%m-%d %H:%M:%S]")
                encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}收到同班同学提交的课程或班级“{lesson_name}”的签到二维码与指定位置信息，但自动签到失败，失败原因为“{txt}”，签到二维码可能已过期，请使用微信小程序重新扫描未过期的签到二维码，扫描后系统将继续尝试签到"}))
                await send_message(encrypt)
                        ...
                        try:
                            if "mobilelearn.chaoxing.com/newsign/preSign" in att["att_chat_course"]["url"] or "mobilelearn.chaoxing.com/widget/attendanceSign/student/studentSign" in att["att_chat_course"]["url"]:
                                ...
                                if await check_sign_type(uid, name, str(att["att_chat_course"]["aid"]), sign_type, port):
                                    ...
                                    USER_LIST[uid]["sign_task_list"][str(att["att_chat_course"]["aid"])] = asyncio.create_task(signt(uid, name, att["att_chat_course"]["courseInfo"]["courseid"], att["att_chat_course"]["courseInfo"]["classid"], att["att_chat_course"]["aid"], schoolid, is_numing, sign_num, att["att_chat_course"]["title"], coursename, 1, tid, port))
...
async def send_wechat_message(uid, message):
    try:
        if NODE_STRAT_TIME+240 < time.time():
            encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_wechat", "uid": uid, "message": message}))
            await send_message(encrypt)
    except Exception:
        await record_error_log(traceback.format_exc(), False)
