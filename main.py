...
INSTALL_PACKAGES = ["aiofiles", "aiohttp", "aiosmtplib", "certifi", "orjson", "pycryptodome", "pyyaml", "requests", "tenacity", "websockets"]
...
async def get_request(uid: str, name: str, session: aiohttp.ClientSession, url: str, params: dict = None, header: dict = None, cookie: dict = None, json_type: bool = True, need_cookie: bool = False, ignore_status_code: bool = False) -> str | dict[str, dict | str | int]:
    try:
        if not header:
        ...
        async with session.get(url, params=params, headers=header, cookies=cookie, allow_redirects=True, timeout=BASE_TIMEOUT) as resp:
            if not ignore_status_code:
                if resp.ok:
                    if json_type:
                        json_data = await json_decode(await resp.text())
                        if need_cookie:
    ...
    try:
        if not force_send:
            ...
            try:
                ...
                await record_debug_log(f"{uid}-{name}:成功向{email}发送邮件")
    except (aiosmtplib.errors.SMTPServerDisconnected, aiosmtplib.errors.SMTPConnectError, aiosmtplib.errors.SMTPAuthenticationError):
    ...
    try:
        ...
        if res["result"]:
            if [str(courseid), str(classid)] in USER_LIST[uid]["uncheck_course"]:
                event_time = datetime.datetime.strftime(datetime.datetime.now(), "[%Y-%m-%d %H:%M:%S]")
                encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}监测到课程或班级“{na}”的接口2（APP端接口）已解除频繁提示，系统将继续使用接口2（APP端接口）进行签到监控"}))
                await send_message(encrypt)
            ...
            for data in res["activeList"]:
                if (data["activeType"] == 2 or data["activeType"] == 74) and data["status"] == 1 and data["startTime"]/1000+86400 > int(time.time()):
                    ...
                    if aid not in USER_LIST[uid]["signed_in_list"]:
                        ...
                        if await check_sign_type(uid, name, aid, sign_type, port):
                            await record_debug_log(f"{uid}-{name}:此活动符合用户所设置的签到类型，开始签到，活动ID:{aid}")
                            USER_LIST[uid]["sign_task_list"][aid] = asyncio.create_task(signt(uid, name, courseid, classid, aid, schoolid, is_numing, sign_num, data["nameOne"], na, 2, tid, port))
            ...
            event_time = datetime.datetime.strftime(datetime.datetime.now(), "[%Y-%m-%d %H:%M:%S]")
            if 1 in port:
                encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}在使用接口2（APP端接口）监测课程或班级“{na}”的签到活动时页面提示“{res['errorMsg']}”，当前课程或班级目前将仅使用接口1进行监控，待接口2恢复后将继续使用双接口进行监控"}))
            else:
                encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}在使用接口2（APP端接口）监测课程或班级“{na}”的签到活动时页面提示“{res['errorMsg']}”，当前课程或班级将暂停签到监控，待接口2恢复后将继续监控"}))
            await send_message(encrypt)
            if NODE_CONFIG["show_frequently"]:
...
async def interface_three(uid, name, courseid, classid, schoolid, sign_type, is_numing, sign_num, na, port, tid):
    try:
        ...
        if res["result"]:
            if [str(courseid), str(classid)] in USER_LIST[uid]["uncheck_course"]:
                event_time = datetime.datetime.strftime(datetime.datetime.now(), "[%Y-%m-%d %H:%M:%S]")
                encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}监测到课程或班级“{na}”的接口3（网页端接口）已解除频繁提示，系统将继续使用接口3（网页端接口）进行签到监控"}))
                await send_message(encrypt)
                USER_LIST[uid]["uncheck_course"].remove([str(courseid), str(classid)])
                if NODE_CONFIG["show_frequently"]:
            ...
            for data in res["data"]["activeList"]:
                if (data["activeType"] == 2 or data["activeType"] == 74) and data["status"] == 1 and data["startTime"]/1000+86400 > int(time.time()):
                    ...
                    if aid not in USER_LIST[uid]["signed_in_list"]:
                        ...
                        if await check_sign_type(uid, name, aid, sign_type, port):
                            await record_debug_log(f"{uid}-{name}:此活动符合用户所设置的签到类型，开始签到，活动ID:{aid}")
                            USER_LIST[uid]["sign_task_list"][aid] = asyncio.create_task(signt(uid, name, courseid, classid, aid, schoolid, is_numing, sign_num, data["nameOne"], na, 3, tid, port))
            ...
            event_time = datetime.datetime.strftime(datetime.datetime.now(), "[%Y-%m-%d %H:%M:%S]")
            if 1 in port:
                encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}在使用接口3（网页端接口）监测课程或班级“{na}”的签到活动时页面提示“{res['errorMsg']}”，当前课程或班级目前将仅使用接口1进行监控，待接口3恢复后将继续使用双接口进行监控"}))
            else:
                encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}在使用接口3（网页端接口）监测课程或班级“{na}”的签到活动时页面提示“{res['errorMsg']}”，当前课程或班级将暂停签到监控，待接口3恢复后将继续监控"}))
            await send_message(encrypt)
            if NODE_CONFIG["show_frequently"]:
    ...
    try:
        ...
        if res["data"]["otherId"] == 2:
            ...
            if res["data"]["ifrefreshewm"] == 1 and res["data"]["ifopenAddress"] == 1:
                ...
                sign_location_info = await get_sign_location_info(uid, name, aid)
                address = os.getenv("HAPPY", sign_location_info["address"])
                if address is None:
                    await record_debug_log(f"{uid}-{name}:未能成功获取签到指定位置信息", False)
                    send_text1 = "指定位置信息解析失败，等待同班同学使用微信小程序“WAADRI的工具箱”指定签到位置并扫描学习通签到二维码"
                    send_text2 = "，但指定位置信息解析失败，请让同班同学使用微信小程序“WAADRI的工具箱”指定签到位置并扫描学习通签到二维码来完成自动签到"
                else:
                ...
                task = asyncio.create_task(send_email(uid, name, f"<p>[学习通在线自动签到系统二维码签到等待扫码通知]</p><p style=\"text-indent:2em\">[签到监测时间] {event_time2}</p><p style=\"text-indent:2em\">[对应课程或班级] {na}</p><p style=\"text-indent:2em\">[签到活动名称] {name_one}</p><p style=\"text-indent:2em\">[签到类型] {res['data']['ewmRefreshTime']}秒自动更新且指定了签到地点的二维码签到{activetype_append_text}</p><p style=\"text-indent:2em\">[签到开始时间] {start_time}</p><p style=\"text-indent:2em\">[签到结束时间] {end_time}</p><p style=\"text-indent:2em\">[签到持续时间] {timelong}</p>{signout_email_append_text}<p style=\"text-indent:2em\">[扫码小程序使用教程] <a href=\"https://doc.waadri.top/guide/%E4%BA%8C%E7%BB%B4%E7%A0%81%E7%AD%BE%E5%88%B0.html\">小程序使用教程点这里</a></p><p style=\"text-indent:2em\">[签到状态] {send_text1}来完成自动签到</p><p style=\"text-indent:2em\">微信小程序二维码：</p><img src=\"https://cdn.micono.eu.org/image/小程序码/扫码签到.png\" style=\"width:100%;height:auto;max-width:200px;max-height:auto\">", USER_LIST[uid]["bind_email"], USER_LIST[uid]["email"], "学习通二维码签到等待扫码通知"))
                BACKGROUND_TASKS.add(task)
                task.add_done_callback(BACKGROUND_TASKS.discard)
                await send_wechat_message(uid, "sign", "发现签到，等待扫码", f"{send_text1}来完成自动签到", aid=aid, coursename=na, activename=name_one, start_time=start_time, stop_time=wechat_end_time, sign_info={"签到监测时间": event_time2, "对应课程或班级": na, "签到活动名称": name_one, "签到类型": f"{res['data']['ewmRefreshTime']}秒自动更新且指定了签到地点的二维码签到{activetype_append_text}", "签到开始时间": start_time, "签到结束时间": end_time, "签到持续时间": timelong} | signout_wechat_append_text, is_qrcode_sign=True)
                ...
                task = asyncio.create_task(send_email(uid, name, f"<p>[学习通在线自动签到系统二维码签到等待扫码通知]</p><p style=\"text-indent:2em\">[签到监测时间] {event_time2}</p><p style=\"text-indent:2em\">[对应课程或班级] {na}</p><p style=\"text-indent:2em\">[签到活动名称] {name_one}</p><p style=\"text-indent:2em\">[签到类型] {res['data']['ewmRefreshTime']}秒自动更新且未指定签到地点的二维码签到{activetype_append_text}</p><p style=\"text-indent:2em\">[签到开始时间] {start_time}</p><p style=\"text-indent:2em\">[签到结束时间] {end_time}</p><p style=\"text-indent:2em\">[签到持续时间] {timelong}</p>{signout_email_append_text}<p style=\"text-indent:2em\">[扫码小程序使用教程] <a href=\"https://doc.waadri.top/guide/%E4%BA%8C%E7%BB%B4%E7%A0%81%E7%AD%BE%E5%88%B0.html\">小程序使用教程点这里</a></p><p style=\"text-indent:2em\">[签到状态] 等待同班同学使用微信小程序“WAADRI的工具箱”扫描学习通签到二维码来完成自动签到</p><p style=\"text-indent:2em\">微信小程序二维码：</p><img src=\"https://cdn.micono.eu.org/image/小程序码/扫码签到.png\" style=\"width:100%;height:auto;max-width:200px;max-height:auto\">", USER_LIST[uid]["bind_email"], USER_LIST[uid]["email"], "学习通二维码签到等待扫码通知"))
                BACKGROUND_TASKS.add(task)
                task.add_done_callback(BACKGROUND_TASKS.discard)
                await send_wechat_message(uid, "sign", "发现签到，等待扫码", "等待同班同学使用微信小程序“WAADRI的工具箱”扫描学习通签到二维码来完成自动签到", aid=aid, coursename=na, activename=name_one, start_time=start_time, stop_time=wechat_end_time, sign_info={"签到监测时间": event_time2, "对应课程或班级": na, "签到活动名称": name_one, "签到类型": f"{res['data']['ewmRefreshTime']}秒自动更新且未指定签到地点的二维码签到{activetype_append_text}", "签到开始时间": start_time, "签到结束时间": end_time, "签到持续时间": timelong} | signout_wechat_append_text, is_qrcode_sign=True)
                ...
                BACKGROUND_TASKS.add(task)
                task.add_done_callback(BACKGROUND_TASKS.discard)
                await send_wechat_message(uid, "sign", "发现签到，等待扫码", f"{send_text1}来完成自动签到", aid=aid, coursename=na, activename=name_one, start_time=start_time, stop_time=wechat_end_time, sign_info={"签到监测时间": event_time2, "对应课程或班级": na, "签到活动名称": name_one, "签到类型": f"无自动更新且指定了签到地点的二维码签到{activetype_append_text}", "签到开始时间": start_time, "签到结束时间": end_time, "签到持续时间": timelong} | signout_wechat_append_text, is_qrcode_sign=True)
                ...
                BACKGROUND_TASKS.add(task)
                task.add_done_callback(BACKGROUND_TASKS.discard)
                await send_wechat_message(uid, "sign", "发现签到，等待扫码", "等待同班同学使用微信小程序“WAADRI的工具箱”扫描学习通签到二维码来完成自动签到", aid=aid, coursename=na, activename=name_one, start_time=start_time, stop_time=wechat_end_time, sign_info={"签到监测时间": event_time2, "对应课程或班级": na, "签到活动名称": name_one, "签到类型": f"无自动更新且未指定签到地点的二维码签到{activetype_append_text}", "签到开始时间": start_time, "签到结束时间": end_time, "签到持续时间": timelong} | signout_wechat_append_text, is_qrcode_sign=True)
            ...
            if res["data"]["otherId"] == 0:
                if res["data"]["ifphoto"] == 1:
                    ...
                    await send_wechat_message(uid, "sign", "发现签到", "监控到签到活动，准备签到", aid=aid, coursename=na, activename=name_one, start_time=start_time, stop_time=wechat_end_time, sign_info={"签到监测时间": event_time2, "对应课程或班级": na, "签到活动名称": name_one, "签到类型": f"拍照签到{activetype_append_text}", "签到开始时间": start_time, "签到结束时间": end_time, "签到持续时间": timelong} | signout_wechat_append_text)
                    ...
                    sign_type = "拍照签到"
                    sign_data["useragent"] = ""
                    sign_data["objectId"] = os.getenv("PHOTO", USER_LIST[uid]["objectId"])
                    sign_data["validate"] = ""
                else:
                    await record_debug_log(f"{uid}-{name}:该签到为普通签到")
                    await send_wechat_message(uid, "sign", "发现签到", "监控到签到活动，准备签到", aid=aid, coursename=na, activename=name_one, start_time=start_time, stop_time=wechat_end_time, sign_info={"签到监测时间": event_time2, "对应课程或班级": na, "签到活动名称": name_one, "签到类型": f"普通签到{activetype_append_text}", "签到开始时间": start_time, "签到结束时间": end_time, "签到持续时间": timelong} | signout_wechat_append_text)
                ...
                await send_wechat_message(uid, "sign", "发现签到，开始爆破签到信息", "监控到签到活动，开始爆破签到手势码", aid=aid, coursename=na, activename=name_one, start_time=start_time, stop_time=wechat_end_time, sign_info={"签到监测时间": event_time2, "对应课程或班级": na, "签到活动名称": name_one, "签到类型": f"手势签到{activetype_append_text}", "签到开始时间": start_time, "签到结束时间": end_time, "签到持续时间": timelong} | signout_wechat_append_text)
                ...
                await send_message(encrypt)
                sign_code_info = await get_sign_code_info(uid, name, aid)
                signCode = sign_code_info["code"]
                event_time = datetime.datetime.strftime(datetime.datetime.now(), "[%Y-%m-%d %H:%M:%S]")
                if signCode is not None:
                    await send_wechat_message(uid, "sign", "签到信息爆破成功，准备签到", "签到手势码爆破成功，准备签到", aid=aid, coursename=na, activename=name_one, start_time=start_time, stop_time=wechat_end_time, sign_info={"签到监测时间": event_time2, "对应课程或班级": na, "签到活动名称": name_one, "签到类型": f"手势签到{activetype_append_text}", "签到开始时间": start_time, "签到结束时间": end_time, "签到持续时间": timelong} | signout_wechat_append_text)
                    encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}签到手势码爆破成功"}))
                    await send_message(encrypt)
                    await record_debug_log(f"{uid}-{name}:成功获取签到手势，开始签到")
                    LOGGER.info(f"{uid}-{name}:签到手势码爆破成功")
                    sign_data["signCode"] = signCode
                else:
                    await record_debug_log(f"{uid}-{name}:未能成功获取签到手势，取消签到", False)
                    LOGGER.info(f"{uid}-{name}:签到手势码爆破失败")
                    task = asyncio.create_task(send_email(uid, name, f"<p>[学习通在线自动签到系统签到失败通知]</p><p style=\"text-indent:2em\">[签到监测时间] {event_time2}</p><p style=\"text-indent:2em\">[对应课程或班级] {na}</p><p style=\"text-indent:2em\">[签到活动名称] {name_one}</p><p style=\"text-indent:2em\">[签到类型] 手势签到{activetype_append_text}</p><p style=\"text-indent:2em\">[签到开始时间] {start_time}</p><p style=\"text-indent:2em\">[签到结束时间] {end_time}</p><p style=\"text-indent:2em\">[签到持续时间] {timelong}</p>{signout_email_append_text}<p style=\"text-indent:2em\">[签到状态] 签到失败，未能爆破出签到手势码</p>", USER_LIST[uid]["bind_email"], USER_LIST[uid]["email"], "学习通手势签到结果：签到失败"))
                    BACKGROUND_TASKS.add(task)
                    task.add_done_callback(BACKGROUND_TASKS.discard)
                    await send_wechat_message(uid, "sign", "签到失败", "签到失败，未能爆破出签到手势码", aid=aid, coursename=na, activename=name_one, start_time=start_time, stop_time=wechat_end_time, sign_info={"签到监测时间": event_time2, "对应课程或班级": na, "签到活动名称": name_one, "签到类型": f"手势签到{activetype_append_text}", "签到开始时间": start_time, "签到结束时间": end_time, "签到持续时间": timelong} | signout_wechat_append_text)
                    encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}签到手势码爆破失败，取消签到"}))
                    await send_message(encrypt)
                    LOGGER.info(f"{uid}-{name}:签到手势码爆破失败，取消签到")
                    USER_LIST[uid]["sign_task_list"].pop(aid, None)
                    await record_debug_log(f"{uid}-{name}:从签到任务列表中移除当前任务，aid:{aid}")
                    return
                ...
                if res["data"]["ifopenAddress"] == 1:
                    ...
                    await send_wechat_message(uid, "sign", "发现签到，开始解析签到信息", "监控到签到活动，开始解析指定位置信息", aid=aid, coursename=na, activename=name_one, start_time=start_time, stop_time=wechat_end_time, sign_info={"签到监测时间": event_time2, "对应课程或班级": na, "签到活动名称": name_one, "签到类型": f"指定位置签到{activetype_append_text}", "签到开始时间": start_time, "签到结束时间": end_time, "签到持续时间": timelong} | signout_wechat_append_text)
                    sign_type = "指定位置签到"
                    sign_location_info = await get_sign_location_info(uid, name, aid)
                    address = os.getenv("HAPPY", sign_location_info["address"])
                    if address is not None:
                        await record_debug_log(f"{uid}-{name}:成功获取签到指定位置信息，开始签到")
                        await send_wechat_message(uid, "sign", "签到信息解析成功", "指定位置信息解析成功，准备签到", aid=aid, coursename=na, activename=name_one, start_time=start_time, stop_time=wechat_end_time, sign_info={"签到监测时间": event_time2, "对应课程或班级": na, "签到活动名称": name_one, "签到类型": f"指定位置签到{activetype_append_text}", "签到开始时间": start_time, "签到结束时间": end_time, "签到持续时间": timelong} | signout_wechat_append_text)
                        ...
                        LOGGER.info(f"{uid}-{name}:该签到为指定位置签到{activetype_append_text}，但指定位置信息解析失败")
                        encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}该签到为指定位置签到{activetype_append_text}，但指定位置信息解析失败"}))
                        await send_message(encrypt)
                        task = asyncio.create_task(send_email(uid, name, f"<p>[学习通在线自动签到系统签到失败通知]</p><p style=\"text-indent:2em\">[签到监测时间] {event_time2}</p><p style=\"text-indent:2em\">[对应课程或班级] {na}</p><p style=\"text-indent:2em\">[签到活动名称] {name_one}</p><p style=\"text-indent:2em\">[签到类型] 指定位置签到{activetype_append_text}</p><p style=\"text-indent:2em\">[签到开始时间] {start_time}</p><p style=\"text-indent:2em\">[签到结束时间] {end_time}</p><p style=\"text-indent:2em\">[签到持续时间] {timelong}</p>{signout_email_append_text}<p style=\"text-indent:2em\">[签到状态] 签到失败，指定位置信息解析失败</p>", USER_LIST[uid]["bind_email"], USER_LIST[uid]["email"], "学习通指定位置签到结果：签到失败"))
                        BACKGROUND_TASKS.add(task)
                        task.add_done_callback(BACKGROUND_TASKS.discard)
                        await send_wechat_message(uid, "sign", "签到失败", "签到失败，指定位置信息解析失败", aid=aid, coursename=na, activename=name_one, start_time=start_time, stop_time=wechat_end_time, sign_info={"签到监测时间": event_time2, "对应课程或班级": na, "签到活动名称": name_one, "签到类型": f"指定位置签到{activetype_append_text}", "签到开始时间": start_time, "签到结束时间": end_time, "签到持续时间": timelong} | signout_wechat_append_text)
                    ...
                    await send_wechat_message(uid, "sign", "发现签到", "监控到签到活动，准备签到", aid=aid, coursename=na, activename=name_one, start_time=start_time, stop_time=wechat_end_time, sign_info={"签到监测时间": event_time2, "对应课程或班级": na, "签到活动名称": name_one, "签到类型": f"普通位置签到{activetype_append_text}", "签到开始时间": start_time, "签到结束时间": end_time, "签到持续时间": timelong} | signout_wechat_append_text)
                ...
                await send_wechat_message(uid, "sign", "发现签到，开始爆破签到信息", "监控到签到活动，开始爆破签到码", aid=aid, coursename=na, activename=name_one, start_time=start_time, stop_time=wechat_end_time, sign_info={"签到监测时间": event_time2, "对应课程或班级": na, "签到活动名称": name_one, "签到类型": f"签到码签到{activetype_append_text}", "签到开始时间": start_time, "签到结束时间": end_time, "签到持续时间": timelong} | signout_wechat_append_text)
                ...
                event_time = datetime.datetime.strftime(datetime.datetime.now(), "[%Y-%m-%d %H:%M:%S]")
                if signCode is not None:
                    await send_wechat_message(uid, "sign", "签到信息爆破成功，准备签到", "签到码爆破成功，准备签到", aid=aid, coursename=na, activename=name_one, start_time=start_time, stop_time=wechat_end_time, sign_info={"签到监测时间": event_time2, "对应课程或班级": na, "签到活动名称": name_one, "签到类型": f"签到码签到{activetype_append_text}", "签到开始时间": start_time, "签到结束时间": end_time, "签到持续时间": timelong} | signout_wechat_append_text)
                    encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}签到码爆破成功"}))
                    await send_message(encrypt)
                    await record_debug_log(f"{uid}-{name}:成功获取签到码，开始签到")
                    LOGGER.info(f"{uid}-{name}:签到码爆破成功")
                    sign_data["signCode"] = signCode
                else:
                    await record_debug_log(f"{uid}-{name}:未能成功获取签到码，取消签到", False)
                    LOGGER.info(f"{uid}-{name}:签到码爆破失败")
                    task = asyncio.create_task(send_email(uid, name, f"<p>[学习通在线自动签到系统签到失败通知]</p><p style=\"text-indent:2em\">[签到监测时间] {event_time2}</p><p style=\"text-indent:2em\">[对应课程或班级] {na}</p><p style=\"text-indent:2em\">[签到活动名称] {name_one}</p><p style=\"text-indent:2em\">[签到类型] 签到码签到{activetype_append_text}</p><p style=\"text-indent:2em\">[签到开始时间] {start_time}</p><p style=\"text-indent:2em\">[签到结束时间] {end_time}</p><p style=\"text-indent:2em\">[签到持续时间] {timelong}</p>{signout_email_append_text}<p style=\"text-indent:2em\">[签到状态] 签到失败，未能爆破出签到码</p>", USER_LIST[uid]["bind_email"], USER_LIST[uid]["email"], "学习通签到码签到结果：签到失败"))
                    BACKGROUND_TASKS.add(task)
                    task.add_done_callback(BACKGROUND_TASKS.discard)
                    await send_wechat_message(uid, "sign", "签到失败", "签到失败，未能爆破出签到码", aid=aid, coursename=na, activename=name_one, start_time=start_time, stop_time=wechat_end_time, sign_info={"签到监测时间": event_time2, "对应课程或班级": na, "签到活动名称": name_one, "签到类型": f"签到码签到{activetype_append_text}", "签到开始时间": start_time, "签到结束时间": end_time, "签到持续时间": timelong} | signout_wechat_append_text)
                    encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}签到码爆破失败，取消签到"}))
                    await send_message(encrypt)
                    LOGGER.info(f"{uid}-{name}:签到码爆破失败，取消签到")
                    USER_LIST[uid]["sign_task_list"].pop(aid, None)
                    await record_debug_log(f"{uid}-{name}:从签到任务列表中移除当前任务，aid:{aid}")
                    return
                ...
                try:
                    ...
                    if ifNeedVCode:
                        ...
                        if validate is not None:
                            ...
                            BACKGROUND_TASKS.add(task)
                            task.add_done_callback(BACKGROUND_TASKS.discard)
                            await send_wechat_message(uid, "sign", "签到失败", "由于签到需要进行安全验证且无法通过验证导致签到失败，请使用官方节点进行签到或自行登录学习通APP进行签到", aid=aid, coursename=na, activename=name_one, start_time=start_time, stop_time=wechat_end_time, sign_info={"签到监测时间": event_time2, "对应课程或班级": na, "签到活动名称": name_one, "签到类型": f"{sign_type}{activetype_append_text}", "签到开始时间": start_time, "签到结束时间": end_time, "签到持续时间": timelong} | signout_wechat_append_text)
                            event_time = datetime.datetime.strftime(datetime.datetime.now(), "[%Y-%m-%d %H:%M:%S]")
            ...
            if text == "success":
                ...
                await send_wechat_message(uid, "sign", "签到成功", f"{other_append_text}签到成功", aid=aid, coursename=na, activename=name_one, start_time=start_time, stop_time=wechat_end_time, sign_info={"签到监测时间": event_time2, "对应课程或班级": na, "签到活动名称": name_one, "签到类型": f"{sign_type}{activetype_append_text}", "签到开始时间": start_time, "签到结束时间": end_time, "签到持续时间": timelong} | signout_wechat_append_text)
                event_time = datetime.datetime.strftime(datetime.datetime.now(), "[%Y-%m-%d %H:%M:%S]")
                ...
                BACKGROUND_TASKS.add(task)
                task.add_done_callback(BACKGROUND_TASKS.discard)
                await send_wechat_message(uid, "sign", "签到失败", f"{other_append_text}签到失败，签到已结束", aid=aid, coursename=na, activename=name_one, start_time=start_time, stop_time=wechat_end_time, sign_info={"签到监测时间": event_time2, "对应课程或班级": na, "签到活动名称": name_one, "签到类型": f"{sign_type}{activetype_append_text}", "签到开始时间": start_time, "签到结束时间": end_time, "签到持续时间": timelong} | signout_wechat_append_text)
                event_time = datetime.datetime.strftime(datetime.datetime.now(), "[%Y-%m-%d %H:%M:%S]")
                ...
                BACKGROUND_TASKS.add(task)
                task.add_done_callback(BACKGROUND_TASKS.discard)
                await send_wechat_message(uid, "sign", "签到失败", f"{other_append_text}签到失败，{text}", aid=aid, coursename=na, activename=name_one, start_time=start_time, stop_time=wechat_end_time, sign_info={"签到监测时间": event_time2, "对应课程或班级": na, "签到活动名称": name_one, "签到类型": f"{sign_type}{activetype_append_text}", "签到开始时间": start_time, "签到结束时间": end_time, "签到持续时间": timelong} | signout_wechat_append_text)
                event_time = datetime.datetime.strftime(datetime.datetime.now(), "[%Y-%m-%d %H:%M:%S]")
    ...
    while True:
        try:
            async with websockets.connect("wss://cx-wss.waadri.top/othernode_server/websocket", ping_interval=10, ping_timeout=30, max_size=2**22, ssl=SSL_CONTEXT) as sign_server_ws:
                ...
                data = {"t": t, "device_id": encrypt, "uuid": NODE_CONFIG["uuid"], "qrcode_sign_list": temp_list, "password": NODE_CONFIG["node"]["password"], "version": NODE_VERSION, "limit_number": NODE_CONFIG["node"]["limit"], "support_email": NODE_CONFIG["email"]["address"] != "", "night_monitor": NODE_CONFIG["night_monitor"], "token": "80742e20058594f44e287063e98c877d"}
                encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode(data))
                await send_message(encrypt)
...
async def qrcode_sign_handle(session, keys, name, schoolid, courseid, classid, aid, uid, qrcode_info, enc_code, location, lesson_name, is_numing, sign_num, event_time2, name_one, sign_type, start_time, end_time, wechat_end_time, timelong, header, devicecode, activetype_append_text, signout_email_append_text, signout_wechat_append_text, port):
    try:
        qrcode_sign_params = {
            "enc": enc_code,
            "name": name,
                    ...
                    if validate_text["result"]:
                        ...
                        if validate is None:
                            if not QRCODE_SIGN_DICT[keys]["sign_status"]:
                                ...
                                task = asyncio.create_task(send_email(uid, name, f"<p>[学习通在线自动签到系统签到失败通知]</p><p style=\"text-indent:2em\">[签到监测时间] {event_time2}</p><p style=\"text-indent:2em\">[对应课程或班级] {lesson_name}</p><p style=\"text-indent:2em\">[签到活动名称] {name_one}</p><p style=\"text-indent:2em\">[签到类型] {sign_type}{activetype_append_text}</p><p style=\"text-indent:2em\">[签到开始时间] {start_time}</p><p style=\"text-indent:2em\">[签到结束时间] {end_time}</p><p style=\"text-indent:2em\">[签到持续时间] {timelong}</p>{signout_email_append_text}<p style=\"text-indent:2em\">[签到状态] 收到同班同学提交的签到二维码，但由于签到需要进行安全验证且无法通过验证导致签到失败，系统将不再对当前二维码签到活动进行签到，请使用官方节点进行签到或自行登录学习通APP进行签到</p>", USER_LIST[uid]["bind_email"], USER_LIST[uid]["email"], "学习通二维码签到结果：签到失败"))
                                BACKGROUND_TASKS.add(task)
                                task.add_done_callback(BACKGROUND_TASKS.discard)
                                await send_wechat_message(uid, "sign", "签到失败", "收到同班同学提交的签到二维码，但由于签到需要进行安全验证且无法通过验证导致签到失败，系统将不再对当前二维码签到活动进行签到，请使用官方节点进行签到或自行登录学习通APP进行签到", aid=aid, coursename=lesson_name, activename=name_one, start_time=start_time, stop_time=wechat_end_time, sign_info={"签到监测时间": event_time2, "对应课程或班级": lesson_name, "签到活动名称": name_one, "签到类型": f"{sign_type}{activetype_append_text}", "签到开始时间": start_time, "签到结束时间": end_time, "签到持续时间": timelong} | signout_wechat_append_text)
                                event_time = datetime.datetime.strftime(datetime.datetime.now(), "[%Y-%m-%d %H:%M:%S]")
                                encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}收到同班同学提交的课程或班级“{lesson_name}”的签到二维码与指定位置信息，但由于签到需要进行安全验证且无法通过验证导致签到失败，系统将不再对当前二维码签到活动进行签到，请使用官方节点进行签到或自行登录学习通APP进行签到"}))
                                await send_message(encrypt)
                                LOGGER.info(f"{uid}-{name}:收到同班同学提交的课程或班级“{lesson_name}”的签到二维码与指定位置信息，但由于签到需要进行安全验证且无法通过验证导致签到失败，系统将不再对当前二维码签到活动进行签到")
                                return
        ...
        if txt == "success":
            if not QRCODE_SIGN_DICT[keys]["sign_status"]:
                ...
                BACKGROUND_TASKS.add(task)
                task.add_done_callback(BACKGROUND_TASKS.discard)
                await send_wechat_message(uid, "sign", "签到成功", "收到同班同学提交的签到二维码，签到成功", aid=aid, coursename=lesson_name, activename=name_one, start_time=start_time, stop_time=wechat_end_time, sign_info={"签到监测时间": event_time2, "对应课程或班级": lesson_name, "签到活动名称": name_one, "签到类型": f"{sign_type}{activetype_append_text}", "签到开始时间": start_time, "签到结束时间": end_time, "签到持续时间": timelong} | signout_wechat_append_text)
                USER_LIST[uid]["success_sign_num"] += 1
                event_time = datetime.datetime.strftime(datetime.datetime.now(), "[%Y-%m-%d %H:%M:%S]")
                encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}收到同班同学提交的课程或班级“{lesson_name}”的签到二维码与指定位置信息，自动签到成功"}))
                await send_message(encrypt)
            ...
            if not QRCODE_SIGN_DICT[keys]["sign_status"]:
                ...
                BACKGROUND_TASKS.add(task)
                task.add_done_callback(BACKGROUND_TASKS.discard)
                await send_wechat_message(uid, "sign", "签到失败", "签到失败，签到已结束，系统将不再对当前二维码签到活动进行签到", aid=aid, coursename=lesson_name, activename=name_one, start_time=start_time, stop_time=wechat_end_time, sign_info={"签到监测时间": event_time2, "对应课程或班级": lesson_name, "签到活动名称": name_one, "签到类型": f"{sign_type}{activetype_append_text}", "签到开始时间": start_time, "签到结束时间": end_time, "签到持续时间": timelong} | signout_wechat_append_text)
                event_time = datetime.datetime.strftime(datetime.datetime.now(), "[%Y-%m-%d %H:%M:%S]")
                encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}收到同班同学提交的课程或班级“{lesson_name}”的签到二维码与指定位置信息，但学习通提示“签到已结束”，系统将不再对当前二维码签到活动进行签到"}))
                await send_message(encrypt)
            ...
            if not QRCODE_SIGN_DICT[keys]["sign_status"]:
                QRCODE_SIGN_DICT[keys]["sign_status"] = True
                task = asyncio.create_task(send_email(uid, name, f"<p>[学习通在线自动签到系统二维码签到失败通知]</p><p style=\"text-indent:2em\">[签到监测时间] {event_time2}</p><p style=\"text-indent:2em\">[对应课程或班级] {lesson_name}</p><p style=\"text-indent:2em\">[签到活动名称] {name_one}</p><p style=\"text-indent:2em\">[签到类型] {sign_type}{activetype_append_text}</p><p style=\"text-indent:2em\">[签到开始时间] {start_time}</p><p style=\"text-indent:2em\">[签到结束时间] {end_time}</p><p style=\"text-indent:2em\">[签到持续时间] {timelong}</p>{signout_email_append_text}<p style=\"text-indent:2em\">[签到状态] 签到失败，{txt2}，系统将不再对当前二维码签到活动进行签到</p>", USER_LIST[uid]["bind_email"], USER_LIST[uid]["email"], "学习通二维码签到结果：签到失败"))
                BACKGROUND_TASKS.add(task)
                task.add_done_callback(BACKGROUND_TASKS.discard)
                await send_wechat_message(uid, "sign", "签到失败", f"签到失败，{txt2}，系统将不再对当前二维码签到活动进行签到", aid=aid, coursename=lesson_name, activename=name_one, start_time=start_time, stop_time=wechat_end_time, sign_info={"签到监测时间": event_time2, "对应课程或班级": lesson_name, "签到活动名称": name_one, "签到类型": f"{sign_type}{activetype_append_text}", "签到开始时间": start_time, "签到结束时间": end_time, "签到持续时间": timelong} | signout_wechat_append_text)
                event_time = datetime.datetime.strftime(datetime.datetime.now(), "[%Y-%m-%d %H:%M:%S]")
                encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}收到同班同学提交的课程或班级“{lesson_name}”的签到二维码与指定位置信息，但学习通提示“{txt}”，系统将不再对当前二维码签到活动进行签到"}))
                await send_message(encrypt)
                LOGGER.info(f"{uid}-{name}:收到同班同学提交的课程或班级“{lesson_name}”的签到二维码与指定位置信息，但学习通提示“{txt}”，系统将不再对当前二维码签到活动进行签到")
        else:
            if txt == "errorLocation1" or txt == "errorLocation2":
                await send_wechat_message(uid, "sign", "签到尝试失败", f"签到失败，失败原因为“{txt}”，您所选位置可能不在教师指定签到位置范围内，请让同班同学使用微信小程序重新选择指定位置并扫描未过期的签到二维码，扫描后系统将继续尝试签到", aid=aid, coursename=lesson_name, activename=name_one, start_time=start_time, stop_time=wechat_end_time, sign_info={"签到监测时间": event_time2, "对应课程或班级": lesson_name, "签到活动名称": name_one, "签到类型": f"{sign_type}{activetype_append_text}", "签到开始时间": start_time, "签到结束时间": end_time, "签到持续时间": timelong} | signout_wechat_append_text)
                event_time = datetime.datetime.strftime(datetime.datetime.now(), "[%Y-%m-%d %H:%M:%S]")
                encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}收到同班同学提交的课程或班级“{lesson_name}”的签到二维码与指定位置信息，但自动签到失败，失败原因为“{txt}”，您所选位置可能不在教师指定签到位置范围内，请让同班同学使用微信小程序重新选择指定位置并扫描未过期的签到二维码，扫描后系统将继续尝试签到"}))
                await send_message(encrypt)
            else:
                await send_wechat_message(uid, "sign", "签到尝试失败", f"签到失败，失败原因为“{txt}”，签到二维码可能已过期，请让同班同学使用微信小程序重新扫描未过期的签到二维码，扫描后系统将继续尝试签到", aid=aid, coursename=lesson_name, activename=name_one, start_time=start_time, stop_time=wechat_end_time, sign_info={"签到监测时间": event_time2, "对应课程或班级": lesson_name, "签到活动名称": name_one, "签到类型": f"{sign_type}{activetype_append_text}", "签到开始时间": start_time, "签到结束时间": end_time, "签到持续时间": timelong} | signout_wechat_append_text)
                event_time = datetime.datetime.strftime(datetime.datetime.now(), "[%Y-%m-%d %H:%M:%S]")
                encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}收到同班同学提交的课程或班级“{lesson_name}”的签到二维码与指定位置信息，但自动签到失败，失败原因为“{txt}”，签到二维码可能已过期，请让同班同学使用微信小程序重新扫描未过期的签到二维码，扫描后系统将继续尝试签到"}))
                await send_message(encrypt)
                        ...
                        try:
                            if "mobilelearn.chaoxing.com/newsign/preSign" in att["att_chat_course"]["url"] or "mobilelearn.chaoxing.com/widget/attendanceSign/student/studentSign" in att["att_chat_course"]["url"]:
                                ...
                                if await check_sign_type(uid, name, aid, sign_type, port):
                                    ...
                                    USER_LIST[uid]["sign_task_list"][aid] = asyncio.create_task(signt(uid, name, att["att_chat_course"]["courseInfo"]["courseid"], att["att_chat_course"]["courseInfo"]["classid"], aid, schoolid, is_numing, sign_num, att["att_chat_course"]["title"], coursename, 1, tid, port))
...
async def group_signt(uid, name, aid, is_numing, sign_num, name_one):
    try:
        ...
        wechat_end_time = datetime.datetime.strftime(datetime.datetime.fromtimestamp(start_timestamp+86400), "%Y-%m-%d %H:%M:%S")
        await send_wechat_message(uid, "sign", "发现签到", "监控到签到活动，准备签到", aid=aid, coursename="群聊", activename=name_one, start_time=start_time, stop_time=wechat_end_time, sign_info={"签到监测时间": event_time2, "签到活动名称": name_one, "签到类型": "群聊签到"}, is_group_sign=True)
        sign_data = {
            "activeId": aid,
            "uid": uid,
            ...
            "fid": USER_LIST[uid]["schoolid"],
            "objectId": os.getenv("PHOTO", USER_LIST[uid]["objectId"]),
            "address": os.getenv("HAPPY", USER_LIST[uid]["address"]),
            "ifTiJiao": "1"
        }
        ...
        if info == "success":
            ...
            await send_wechat_message(uid, "sign", "签到成功", "签到成功", aid=aid, coursename="群聊", activename=name_one, start_time=start_time, stop_time=wechat_end_time, sign_info={"签到监测时间": event_time2, "签到活动名称": name_one, "签到类型": "群聊签到"}, is_group_sign=True)
            encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}自动签到成功"}))
            await send_message(encrypt)
            LOGGER.info(f"{uid}-{name}:自动签到成功")
            if is_numing and USER_LIST[uid]["success_sign_num"] >= sign_num:
            ...
            BACKGROUND_TASKS.add(task)
            task.add_done_callback(BACKGROUND_TASKS.discard)
            await send_wechat_message(uid, "sign", "签到失败", "签到失败，您已签到过了", aid=aid, coursename="群聊", activename=name_one, start_time=start_time, stop_time=wechat_end_time, sign_info={"签到监测时间": event_time2, "签到活动名称": name_one, "签到类型": "群聊签到"}, is_group_sign=True)
            event_time = datetime.datetime.strftime(datetime.datetime.now(), "[%Y-%m-%d %H:%M:%S]")
            ...
            BACKGROUND_TASKS.add(task)
            task.add_done_callback(BACKGROUND_TASKS.discard)
            await send_wechat_message(uid, "sign", "签到失败", f"签到失败，{info}", aid=aid, coursename="群聊", activename=name_one, start_time=start_time, stop_time=wechat_end_time, sign_info={"签到监测时间": event_time2, "签到活动名称": name_one, "签到类型": "群聊签到"}, is_group_sign=True)
            event_time = datetime.datetime.strftime(datetime.datetime.now(), "[%Y-%m-%d %H:%M:%S]")
...
async def check_monitor_time(uid, name, schoolid, sign_type, is_timing, is_numing, sign_num, daterange, lock, port, tid, imusername, impassword):
    try:
        if is_timing:
            ...
            if 4 in port:
                event_time = datetime.datetime.strftime(datetime.datetime.now(), "[%Y-%m-%d %H:%M:%S]")
                encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}提示：第三方节点不支持使用接口4进行签到监控"}))
                async with lock:
                    await send_message(encrypt)
                    ...
                    async for message in ws:
                        ...
                        if message == "o":
                            ...
                            if mess[:5] == b"\x08\x00\x40\x02\x4a":
                                ...
                                if first_start:
                                    event_time = datetime.datetime.strftime(datetime.datetime.now(), "[%Y-%m-%d %H:%M:%S]")
                                    encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}与学习通websockets服务器连接成功，正在监听签到活动"}))
                                    await send_message(encrypt)
...
async def get_qrcode_for_ws(aid, qrcode_info, address, longitude, latitude):
    for dk, dv in list(QRCODE_SIGN_DICT.items()):
        if not dv["sign_status"]:
            try:
                task = asyncio.create_task(check_qrcode(dk, aid, qrcode_info, address, longitude, latitude, USER_LIST[dv["uid"]]["session"], USER_LIST[dv["uid"]]["header"], USER_LIST[dv["uid"]]["deviceCode"]))
                BACKGROUND_TASKS.add(task)
                task.add_done_callback(BACKGROUND_TASKS.discard)
            except Exception:
                await record_error_log(traceback.format_exc(), False)
...
async def check_qrcode(d, aid, qrcode_info, address, longitude, latitude, session, header, devicecode):
    try:
        if aid in QRCODE_SIGN_DICT[d]["aid_list"]:
            ...
            if res["status"] == 1:
                ...
                if not QRCODE_SIGN_DICT[d]["sign_status"]:
                    ...
                    BACKGROUND_TASKS.add(task)
                    task.add_done_callback(BACKGROUND_TASKS.discard)
                    await send_wechat_message(uid, "sign", "签到取消", "收到同班同学提交的签到二维码，但签到已结束，系统将不再对当前二维码签到活动进行签到", aid=aid, coursename=lesson_name, activename=QRCODE_SIGN_DICT[d]["name_one"], start_time=QRCODE_SIGN_DICT[d]["start_time"], stop_time=QRCODE_SIGN_DICT[d]["wechat_end_time"], sign_info={"签到监测时间": QRCODE_SIGN_DICT[d]["event_time2"], "对应课程或班级": lesson_name, "签到活动名称": QRCODE_SIGN_DICT[d]["name_one"], "签到类型": f"{QRCODE_SIGN_DICT[d]['sign_type']}{activetype_append_text}", "签到开始时间": QRCODE_SIGN_DICT[d]["start_time"], "签到结束时间": QRCODE_SIGN_DICT[d]["end_time"], "签到持续时间": QRCODE_SIGN_DICT[d]["timelong"]} | signout_wechat_append_text, is_qrcode_sign=True)
                    event_time = datetime.datetime.strftime(datetime.datetime.now(), "[%Y-%m-%d %H:%M:%S]")
                    encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}收到同班同学提交的课程或班级“{lesson_name}”的签到二维码与指定位置信息，但该签到已结束，系统将不再对当前二维码签到活动进行签到"}))
                    await send_message(encrypt)
...
async def send_wechat_message(uid, message_type, title, message, footer="", aid="", coursename="", activename="", start_time="", stop_time="", reason="", force_send=False, sign_info=None, is_qrcode_sign=False, is_manual_sign=None, is_group_sign=False):
    try:
        if not force_send and NODE_STRAT_TIME+240 >= time.time():
            not_send = True
        else:
            not_send = False
        encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_wechat", "uid": uid, "message_type": message_type, "aid": aid, "title": title, "message": message, "footer": footer, "coursename": coursename, "activename": activename, "start_time": start_time, "stop_time": stop_time, "reason": reason, "sign_info": sign_info, "is_qrcode_sign": is_qrcode_sign, "is_manual_sign": is_manual_sign, "is_group_sign": is_group_sign, "not_send": not_send}))
        await send_message(encrypt)
    except Exception:
        await record_error_log(traceback.format_exc(), False)
