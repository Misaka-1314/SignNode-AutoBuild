...
async def get_request(uid: str, name: str, session: aiohttp.ClientSession, url: str, params: dict = None, header: dict = None, cookie: dict = None, json_type: bool = True, need_cookie: bool = False, ignore_status_code: bool = False, need_response: bool = False) -> str | dict[str, dict | str | int]:
    try:
        if not header:
                ...
                if resp.status == 502:
                    ...
                    try:
                        ...
                        if not need_response:
                            ...
                            return {"result": 0, "status": 0, "errorMsg": await resp.text()}
                else:
                    return await resp.text()
        ...
        if 2 in USER_LIST[uid]["port"]:
            ...
            if res["result"]:
                ...
                if USER_LIST[uid]["error_num"] < 2:
                    ...
                    event_time = datetime.datetime.strftime(datetime.datetime.now(), "[%Y-%m-%d %H:%M:%S]")
                    encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}在使用接口2（APP端接口）监控课程或班级“{na}”的签到活动时页面提示“{res['errorMsg']}”，系统将尝试使用接口3（网页端接口）进行签到监控"}))
                    await send_message(encrypt)
...
async def interface_three(uid, name, clazzdata, schoolid, sign_type, is_numing, sign_num, na, tid):
    try:
        if 3 in USER_LIST[uid]["port"]:
            ...
            res = await get_request(uid, name, USER_LIST[uid]["session"], "https://mobilelearn.chaoxing.com/v2/apis/active/student/activelist", {"fid": fid, "courseId": clazzdata["courseid"], "classId": clazzdata["classid"], "showNotStartedActive": 0, "_": int(time.time()*1000)}, ignore_status_code=True)
            if res["result"]:
                USER_LIST[uid]["error_num"] = 0
                ...
                if USER_LIST[uid]["error_num"] < 2:
                    ...
                    event_time = datetime.datetime.strftime(datetime.datetime.now(), "[%Y-%m-%d %H:%M:%S]")
                    encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}在使用接口3（网页端接口）监控课程或班级“{na}”的签到活动时页面提示“{res['errorMsg']}”，系统将尝试使用接口4（主备用接口）进行签到监控"}))
                    await send_message(encrypt)
                    if NODE_CONFIG["show_frequently"]:
    ...
    try:
        ...
        if res["result"]:
            ...
            if USER_LIST[uid]["error_num"] < 2:
                ...
                event_time = datetime.datetime.strftime(datetime.datetime.now(), "[%Y-%m-%d %H:%M:%S]")
                encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": uid, "name": name, "message": f"{event_time}在使用接口4（主备用接口）监控课程或班级“{na}”的签到活动时页面提示“{res['errorMsg']}”，系统将尝试使用接口2（APP端接口）进行签到监控"}), SERVER_KEY, SERVER_IV)
                await send_message(encrypt)
                encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode({"type": "send_sign_message", "uid": "113028447", "name": "all_sign", "message": f"{event_time}{uid}-{name}:课程或班级“{na}”的页面提示“{res['errorMsg']}”，将尝试使用接口2（APP端接口）进行签到监控"}), SERVER_KEY, SERVER_IV)
                await send_message(encrypt)
                if NODE_CONFIG["show_frequently"]:
        ...
        if res["data"]["otherId"] == 2:
            ...
            if res["data"]["otherId"] == 0:
                ...
                if signCode is not None:
                    await send_wechat_message(uid, "sign", "签到信息爆破成功，准备签到", "签到手势码爆破成功，准备签到", icon="time", aid=aid, coursename=na, activename=name_one, start_time=start_time, stop_time=wechat_end_time, sign_info={"签到监测时间": event_time2, "对应课程或班级": na, "签到活动名称": name_one, "签到类型": f"手势签到{activetype_append_text}", "签到开始时间": start_time, "签到结束时间": end_time, "签到持续时间": timelong} | signout_wechat_append_text)
                    ...
                    BACKGROUND_TASKS.add(task)
                    task.add_done_callback(BACKGROUND_TASKS.discard)
                    await send_wechat_message(uid, "sign", "签到失败", "签到失败，未能爆破出签到手势码", icon="close-octagon", aid=aid, coursename=na, activename=name_one, start_time=start_time, stop_time=wechat_end_time, sign_info={"签到监测时间": event_time2, "对应课程或班级": na, "签到活动名称": name_one, "签到类型": f"手势签到{activetype_append_text}", "签到开始时间": start_time, "签到结束时间": end_time, "签到持续时间": timelong} | signout_wechat_append_text)
                ...
                if res["data"]["ifopenAddress"] == 1:
                    ...
                    if address is not None:
                        ...
                        BACKGROUND_TASKS.add(task)
                        task.add_done_callback(BACKGROUND_TASKS.discard)
                        await send_wechat_message(uid, "sign", "签到失败", "签到失败，指定位置信息解析失败", icon="close-octagon", aid=aid, coursename=na, activename=name_one, start_time=start_time, stop_time=wechat_end_time, sign_info={"签到监测时间": event_time2, "对应课程或班级": na, "签到活动名称": name_one, "签到类型": f"指定位置签到{activetype_append_text}", "签到开始时间": start_time, "签到结束时间": end_time, "签到持续时间": timelong} | signout_wechat_append_text)
                ...
                event_time = datetime.datetime.strftime(datetime.datetime.now(), "[%Y-%m-%d %H:%M:%S]")
                if signCode is not None:
                    await send_wechat_message(uid, "sign", "签到信息爆破成功，准备签到", "签到码爆破成功，准备签到", icon="time", aid=aid, coursename=na, activename=name_one, start_time=start_time, stop_time=wechat_end_time, sign_info={"签到监测时间": event_time2, "对应课程或班级": na, "签到活动名称": name_one, "签到类型": f"签到码签到{activetype_append_text}", "签到开始时间": start_time, "签到结束时间": end_time, "签到持续时间": timelong} | signout_wechat_append_text)
                    ...
                    BACKGROUND_TASKS.add(task)
                    task.add_done_callback(BACKGROUND_TASKS.discard)
                    await send_wechat_message(uid, "sign", "签到失败", "签到失败，未能爆破出签到码", icon="close-octagon", aid=aid, coursename=na, activename=name_one, start_time=start_time, stop_time=wechat_end_time, sign_info={"签到监测时间": event_time2, "对应课程或班级": na, "签到活动名称": name_one, "签到类型": f"签到码签到{activetype_append_text}", "签到开始时间": start_time, "签到结束时间": end_time, "签到持续时间": timelong} | signout_wechat_append_text)
    ...
    while True:
        try:
            async with websockets.connect("wss://cx-wss.waadri.top/othernode_server/websocket", ping_interval=10, ping_timeout=30, max_size=2**22, ssl=SSL_CONTEXT) as sign_server_ws:
                ...
                data = {"t": t, "device_id": encrypt, "uuid": NODE_CONFIG["uuid"], "qrcode_sign_list": temp_list, "password": NODE_CONFIG["node"]["password"], "version": NODE_VERSION, "limit_number": NODE_CONFIG["node"]["limit"], "support_email": NODE_CONFIG["email"]["address"] != "", "night_monitor": NODE_CONFIG["night_monitor"], "token": "e0e9345a04bf4bf2182d79ca9cbeb699"}
                encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode(data))
                await send_message(encrypt)
    ...
    try:
        ...
        if txt == "success":
            ...
            if not QRCODE_SIGN_DICT[keys]["sign_status"]:
                ...
                BACKGROUND_TASKS.add(task)
                task.add_done_callback(BACKGROUND_TASKS.discard)
                await send_wechat_message(uid, "sign", "签到失败", "签到失败，签到已结束，系统将不再对当前二维码签到活动进行签到", icon="close-octagon", aid=aid, coursename=lesson_name, activename=name_one, start_time=start_time, stop_time=wechat_end_time, sign_info={"签到监测时间": event_time2, "对应课程或班级": lesson_name, "签到活动名称": name_one, "签到类型": f"{sign_type}{activetype_append_text}", "签到开始时间": start_time, "签到结束时间": end_time, "签到持续时间": timelong} | signout_wechat_append_text)
                event_time = datetime.datetime.strftime(datetime.datetime.now(), "[%Y-%m-%d %H:%M:%S]")
            ...
            if not QRCODE_SIGN_DICT[keys]["sign_status"]:
                ...
                BACKGROUND_TASKS.add(task)
                task.add_done_callback(BACKGROUND_TASKS.discard)
                await send_wechat_message(uid, "sign", "签到失败", f"签到失败，{txt2}，系统将不再对当前二维码签到活动进行签到", icon="close-octagon", aid=aid, coursename=lesson_name, activename=name_one, start_time=start_time, stop_time=wechat_end_time, sign_info={"签到监测时间": event_time2, "对应课程或班级": lesson_name, "签到活动名称": name_one, "签到类型": f"{sign_type}{activetype_append_text}", "签到开始时间": start_time, "签到结束时间": end_time, "签到持续时间": timelong} | signout_wechat_append_text)
                event_time = datetime.datetime.strftime(datetime.datetime.now(), "[%Y-%m-%d %H:%M:%S]")
            ...
            if txt == "errorLocation1" or txt == "errorLocation2":
                await send_wechat_message(uid, "sign", "签到尝试失败", f"签到失败，失败原因为“{txt}”，您所选位置可能不在教师指定签到位置范围内，请让同班同学使用微信小程序重新选择指定位置并扫描未过期的签到二维码，扫描后系统将继续尝试签到", icon="error-circle", aid=aid, coursename=lesson_name, activename=name_one, start_time=start_time, stop_time=wechat_end_time, sign_info={"签到监测时间": event_time2, "对应课程或班级": lesson_name, "签到活动名称": name_one, "签到类型": f"{sign_type}{activetype_append_text}", "签到开始时间": start_time, "签到结束时间": end_time, "签到持续时间": timelong} | signout_wechat_append_text)
                event_time = datetime.datetime.strftime(datetime.datetime.now(), "[%Y-%m-%d %H:%M:%S]")
...
async def check_sign_type(uid, name, activeid, sign_type):
    ...
    if not res["result"]:
        if res["errorMsg"] == "no data":
            await record_error_log(await json_encode(res))
            await record_error_log(f"{uid}-{name}:签到类型信息获取失败，等待2秒后重试")
            raise ValueError
                ...
                async with websockets.connect(f"wss://im-api-vip6-v2.easemob.com/ws/{ws_str1}/{ws_str2}/websocket", ping_interval=None, ping_timeout=None, ssl=SSL_CONTEXT) as ws:
                    ...
                    async for message in ws:
                        ...
                        if message == "o":
                            ...
                            await ws_login(ws, uid, name, imusername, impassword)
                        elif message[0] == "a":
                            ...
                            if mess[:5] == b"\x08\x00\x40\x02\x4a":
                                ...
                                await get_message(ws, mess, uid, name, schoolid, sign_type, is_numing, sign_num, tid)
        ...
        if aid in QRCODE_SIGN_DICT[d]["aid_list"]:
            ...
            if res["status"] == 1:
                ...
                if not QRCODE_SIGN_DICT[d]["sign_status"]:
                    ...
                    BACKGROUND_TASKS.add(task)
                    task.add_done_callback(BACKGROUND_TASKS.discard)
                    await send_wechat_message(uid, "sign", "签到取消", "收到同班同学提交的签到二维码，但签到已结束，系统将不再对当前二维码签到活动进行签到", icon="error-circle", aid=aid, coursename=lesson_name, activename=QRCODE_SIGN_DICT[d]["name_one"], start_time=QRCODE_SIGN_DICT[d]["start_time"], stop_time=QRCODE_SIGN_DICT[d]["wechat_end_time"], sign_info={"签到监测时间": QRCODE_SIGN_DICT[d]["event_time2"], "对应课程或班级": lesson_name, "签到活动名称": QRCODE_SIGN_DICT[d]["name_one"], "签到类型": f"{QRCODE_SIGN_DICT[d]['sign_type']}{activetype_append_text}", "签到开始时间": QRCODE_SIGN_DICT[d]["start_time"], "签到结束时间": QRCODE_SIGN_DICT[d]["end_time"], "签到持续时间": QRCODE_SIGN_DICT[d]["timelong"]} | signout_wechat_append_text, is_qrcode_sign=True)
                    event_time = datetime.datetime.strftime(datetime.datetime.now(), "[%Y-%m-%d %H:%M:%S]")
