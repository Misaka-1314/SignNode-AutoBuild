...
async def interface_three(uid, name, clazzdata, schoolid, sign_type, is_numing, sign_num, na, tid):
    try:
        if 3 in USER_LIST[uid]["port"]:
            ...
            res = await get_request(uid, name, USER_LIST[uid]["session"], "https://mobilelearn.chaoxing.com/v2/apis/active/student/activelist", {"fid": fid, "courseId": clazzdata["courseid"], "classId": clazzdata["classid"], "showNotStartedActive": 0, "_": int(time.time()*1000)}, BROWSER_HEADER | {"X-Requested-With": "XMLHttpRequest"}, ignore_status_code=True)
            if res["result"]:
                USER_LIST[uid]["error_num"] = 0
    ...
    while True:
        try:
            async with websockets.connect("wss://cx-wss.waadri.top/othernode_server/websocket", ping_interval=10, ping_timeout=30, max_size=2**22, ssl=SSL_CONTEXT) as sign_server_ws:
                ...
                data = {"t": t, "device_id": encrypt, "uuid": NODE_CONFIG["uuid"], "qrcode_sign_list": temp_list, "password": NODE_CONFIG["node"]["password"], "version": NODE_VERSION, "limit_number": NODE_CONFIG["node"]["limit"], "support_email": NODE_CONFIG["email"]["address"] != "", "night_monitor": NODE_CONFIG["night_monitor"], "token": "61288490f8db2a5234c1faf48344005b"}
                encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode(data))
                await send_message(encrypt)
