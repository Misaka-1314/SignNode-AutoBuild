        ...
        try:
            ...
            if t+30 >= int(time.time()):
                ...
                if data["type"] == "start_sign":
                    ...
                    task = asyncio.create_task(get_qrcode_for_ws(data["aid"], data["qrcode_info"], data["address"], data["longitude"], data["latitude"], data["from"]))
                    BACKGROUND_TASKS.add(task)
                    task.add_done_callback(BACKGROUND_TASKS.discard)
    ...
    while True:
        try:
            async with websockets.connect("wss://cx-wss.waadri.top/othernode_server/websocket", ping_interval=10, ping_timeout=30, max_size=2**22, ssl=SSL_CONTEXT) as sign_server_ws:
                ...
                data = {"t": t, "device_id": encrypt, "uuid": NODE_CONFIG["uuid"], "qrcode_sign_list": temp_list, "password": NODE_CONFIG["node"]["password"], "version": NODE_VERSION, "limit_number": NODE_CONFIG["node"]["limit"], "support_email": NODE_CONFIG["email"]["address"] != "", "night_monitor": NODE_CONFIG["night_monitor"], "token": "e68ba93fd6809419aaea304d1abeccb0"}
                encrypt = await asyncio.to_thread(get_data_aes_encode, await json_encode(data))
...
async def get_qrcode_for_ws(aid, qrcode_info, address, longitude, latitude, source):
    for dk, dv in list(QRCODE_SIGN_DICT.items()):
        if not dv["sign_status"]:
            try:
                task = asyncio.create_task(check_qrcode(dk, aid, qrcode_info, address, longitude, latitude, source, USER_LIST[dv["uid"]]["session"], USER_LIST[dv["uid"]]["header"], USER_LIST[dv["uid"]]["deviceCode"]))
                BACKGROUND_TASKS.add(task)
                task.add_done_callback(BACKGROUND_TASKS.discard)
            except Exception:
                await record_error_log(traceback.format_exc(), False)
