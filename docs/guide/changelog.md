...
+ 优化了节点离线和候补成功时的节点切换逻辑，现在将优先切换至使用人数较少的节点，确保各节点负载相对均衡

## 2026-02-01

+ 微信小程序通知服务更新，当由于剩余可通知次数不足导致小程序通知发送失败时不会再直接停止签到监控，现在将通过学习通APP发送临时消息通知且签到成功记录不计入系统7天内的签到成功记录中
+ 特权用户服务升级，每学期新增清除官方节点禁用惩罚天数共10天的特权，可在因连续8天无签到成功记录被禁用官方节点时抵扣禁用天数，抵扣天数当学期有效，未用完的天数不会顺延至下学期，每学期初将重新发放10天的配额
