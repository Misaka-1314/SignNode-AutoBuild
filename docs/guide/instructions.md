...
- 系统在不开启任何签到模式下默认为自动签到模式，此时签到逻辑遵循以下规则：
  ...
  + **手势签到和签到码签到**：自动获取教师设置的手势或签到码，然后为防止教师未公布手势或签到码用户就成功签到，系统会持续监测签到人数，待有同学完成签到后再进行自动签到。
...
- **惩罚规则**：为了保证官方节点资源可以得到充分利用，用户使用官方节点启动签到监控后若连续7天无签到成功记录将自动停止签到监控，被停止后若第二日仍使用过官方节点且无签到成功记录将被禁止使用官方节点1天。
